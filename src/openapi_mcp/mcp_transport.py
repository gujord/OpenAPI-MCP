...
class MCPHttpTransport:
    ...
    def __init__(
        ...
        self.cors_origins = cors_origins if cors_origins is not None else default_cors
    ...
    async def _handle_mcp_request(self, request: Request):
        ...
        try:
            ...
            try:
                ...
                return JSONResponse(
                    {"jsonrpc": "2.0", "error": {"code": -32700, "message": "Parse error", "data": str(e)}},
                    status_code=400,
                )
    ...
    async def _handle_sse_stream(self, request: Request):
        ...
        async def event_generator():
            ...
            yield self._format_sse_event(
                "connected", {"session_id": session_id, "transport": "mcp-http-sse", "connected_at": time.time()}
            )
            ...
            try:
                ...
                while session.active:
                    ...
                    # Send heartbeat
                    yield self._format_sse_event("heartbeat", {"timestamp": time.time(), "session_id": session_id})
                ...
                yield self._format_sse_event("error", {"error": str(e), "session_id": session_id})
            finally:
                # Send disconnect event
                yield self._format_sse_event("disconnected", {"session_id": session_id, "disconnected_at": time.time()})
        ...
        async def mcp_sse_generator():
            ...
            yield self._format_sse_event(
                "connected",
                {
                    "transport": "mcp-http-sse",
                    "session_id": session_id,
                    "server_info": {"name": self.mcp_server.server_name, "version": "1.0.0"},
        ...
        try:
            ...
            if method == "initialize":
                ...
                return self.mcp_server._tools_call_tool(
                    req_id=request_id, name=params.get("name"), arguments=params.get("arguments", {})
                )
    ...
    def _format_sse_event(self, event_type: str, data: Dict[str, Any], event_id: Optional[str] = None) -> str:
